window.onload = function() { // Used in URL proposal loading method.
    ...
    function processJSON(dat) { // Obtains JSON Required Data
            ...
            try {
                ...
                for (var i = 0; i < json["line_items"].length; i++) {
                    ...
                    if (json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["selected"] == true) {
                        ...
                        table += "</td>";
                        table += "<td>";
                        table += "$" + json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["unit_price"].toFixed(2);
                        table += "</td>";
                        table += "<td>";
                        table += json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["units"] <= 0 ? json["number_of_guests"] + " Guests" : json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["units"];
                        table += "</td>";
                        table += "<td>";
                        table += json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["cost"] == 0 ? " " : "$" + json["line_items"][Object.keys(order[i][Object.keys(order[i])])]["cost"].toFixed(2);
                        table += "</td>";
                        table += "</tr>";
            ...
            function processLoc() {
                ...
                if (pid != undefined) {
                    ...
                    try {
                        ...
                        var id = (pid.split('?')[0] != undefined) ? pid.split('?')[0] : pid;
                        var domain = (url.split('?')[1].split('=')[1] != undefined) ? url.split('?')[1].split('=')[1] : "app";
